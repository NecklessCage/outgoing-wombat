...
df['datetime_posted'] = pd.to_datetime(
    df.datetime).dt.tz_convert('Asia/Rangoon')
df['datetime_edited'] = pd.to_datetime(
    df.edit_datetime).dt.tz_convert('Asia/Rangoon')
...
# %%
data = df[['post_url', 'channel_url', 'datetime_posted',
           'message', 'datetime_edited', 'views']]
data.rename(mapper={
    'message': 'msg_og'
}, axis=1, inplace=True)
data.head()
